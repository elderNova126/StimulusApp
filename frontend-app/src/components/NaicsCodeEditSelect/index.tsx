...
import { naicsTooltipBox, styleDropdownList, styleDropdownSubList } from './styles';
...
const NaicsCodeEditSelectWrapper: FC<{
  loading: boolean;
  ...
  naicsIndustries: { industries: any };
}> = ({ loading, selectedNaics, setSelectedNaics, setDeleteNaics, industriesSelected, naicsIndustries }) => {
...
const NaicsCodeSelect: FC<{
  ...
  const [newCustomIndustry, setnewCustomIndustry] = useState<IndustriesTreeRoot>({});
  ...
  const {
    ...
    onInputValueChange: (data) => {
      ...
      if (value) {
        // means next value has more chars
        setIndustriesTree((industriesTree: IndustriesTreeRoot) => {
          return searchTreeView(value.toLocaleLowerCase(), tree);
        });
        setFilteredIndustries((industriesTree: IndustriesTreeRoot) => {
          return searchTreeView(value.toLocaleLowerCase(), filteredIndustries);
        });

        setnewCustomIndustry((industriesTree: IndustriesTreeRoot) => {
          return searchTreeView(value.toLocaleLowerCase(), newCustomIndustry);
        });
      } else {
        // means next value has fewer chars
        setIndustriesTree(tree);
        setFilteredIndustries(customIndustries);
      }
    },
  ...
  useEffect(() => {
    if (Object.keys(industriesTree).length === 0 && Object.keys(filteredIndustries).length === 0) {
      const newCustomIndustry: any = {
        id: '',
        code: null,
        title: inputValue,
        value: inputValue,
      ...
      setnewCustomIndustry({ NEW: newCustomIndustry });
    } else {
      setnewCustomIndustry({});
    }
  }, [inputValue, industriesTree, customIndustries]); // eslint-disable-line react-hooks/exhaustive-deps
  const menuProps = getMenuProps();
  const allOpen = Boolean(inputValue.length > 3);

  const listItems = useMemo(
    () =>
      Object.keys(industriesTree).map((item, index) => {
        return (
          <NaicsListItem
            key={index}
            isNewIndustry={false}
  ...
  const listNewCustomItems = useMemo(
    () =>
      Object.keys(newCustomIndustry)
        .map((item) => {
          return (
            <NaicsListItem
              isNewIndustry={true}
  ...
  return (
    <Stack w="100%" mt="1rem">
      <Flex flexDirection="column" align={'flex-start'} {...getComboboxProps()}>
        {multiSelect && (
          <Flex flexWrap={'wrap'}>
            {selectedNaics
              .filter((industry: IndustryCodeItem) => !!industry.title)
              .map((industry: IndustryCodeItem, index: number) => (
                <Flex
                  margin="0 5px 5px 0"
                ...
                >
                  <CodeView
                    title={industry?.title}
                    code={industry?.code}
                    customTooltip={industry?.title}
                    p="2px"
                    selected={true}
                    deleteIndustry={() => setDeleteNaics(industry)}
                  />
                </Flex>
              ))}
          </Flex>
      ...
      <Box position="relative" w="100%">
        <List {...((!isOpen || loading) && { display: 'none' })} sx={styleDropdownList} {...menuProps}>
          ...
          >
            ...
            {openList.standard && (
              <Stack sx={styleDropdownSubList('370px', '250px')} {...menuProps}>
          ...
          <Box onClick={() => setOpenList({ standard: false, custom: !openList.custom })} cursor="pointer">
            ...
            {openList.custom && (
              <Stack sx={styleDropdownSubList('370px', '200px')} {...menuProps}>
                {openList.custom && listCustomItems}
                {openList.custom && listNewCustomItems}
              </Stack>
            )}
          </Box>
        </List>
...
const NaicsListItem: FC<{
  ...
  allOpen: boolean;
  isNewIndustry: boolean;
  item: NaicsTreeItem;
  level: number;
  selectedNaics: IndustryCodeItem[];
  industriesSelected: any[];
  checked?: boolean;
}> = ({ level, allOpen, item, selectItem, profile, selectedNaics, isNewIndustry, industriesSelected, checked }) => {
  ...
  if (isCustom) {
    isLeaf = true;
    isChecked = industriesSelected.find((industry) => industry.value === item.value);
  } else {
    isChecked = industriesSelected.find((industry) => industry.code === item.code);
  }

  const checkHaveChildren = (item: NaicsTreeItem) => {
    if (item.children) {
      if (Object.keys(item.children).length === 0) {
        return false;
      }
  ...
  useEffect(() => {
    setHaveChildren(checkHaveChildren(item));
  }, [item]);
...
export default NaicsCodeEditSelectWrapper;
