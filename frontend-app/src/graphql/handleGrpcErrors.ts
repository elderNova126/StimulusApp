...
import { GrpcStatus, INTERNAL_ERROR } from './GrpcStatus.enum';
...
const HandleGrpcHook = () => {
  ...
  const handleGrpcErrors = (err: GraphQLErrors | undefined) => {
    ...
    if (err.length > 0) {
      if (
        (err[0]?.extensions?.code in GrpcStatus &&
          err[0]?.extensions?.details === INTERNAL_ERROR.DISCOVERCOMPANIESMAP) ||
        err[0]?.extensions?.details === INTERNAL_ERROR.UNAVAILABLE
      ) {
        return;
      }
